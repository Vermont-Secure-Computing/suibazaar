...
import { MARKETPLACE_PACKAGE_ID, ESCROW_PACKAGE_ID, MODULE } from "../config";
...
const MERCHANT_EVENT = `${MARKETPLACE_PACKAGE_ID}::${MODULE}::MerchantCreated`;
...
export async function getMerchantByOwner(owner) {
  ...
  const type = `${MARKETPLACE_PACKAGE_ID}::${MODULE}::MerchantProfile`;
...
export async function getProductsByMerchant(merchantAddress) {
  ...
  const eventType = `${MARKETPLACE_PACKAGE_ID}::${MODULE}::ProductCreated`;
  ...
  while (true) {
    for (const event of page.events ?? []) {
      ...
      if (data?.merchant?.toLowerCase() === merchantAddress.toLowerCase()) {
...
export async function getMerchantByAuthority(authority) {
  ...
  return (
    merchants.find(
      (merchant) =>
        merchant.authority?.toLowerCase() === authority.toLowerCase()
    ) ?? null
  );
}

export async function getEscrowCreatedByDigest(digest) {
  const eventType = `${ESCROW_PACKAGE_ID}::escrow::EscrowCreated`;
  ...
  const event = (result.events ?? []).find(
    (item) =>
      item.transactionDigest === digest || item.transaction?.digest === digest
  );

  if (!event?.json?.escrow_id) {
    throw new Error("Escrow was created but its object ID could not be found.");
  }

  return event.json.escrow_id;
}

const ORDER_EVENT = `${MARKETPLACE_PACKAGE_ID}::${MODULE}::OrderCreated`;

export async function getOrderRecord(objectId) {
  if (!objectId) {
    throw new Error("Order record object ID is required.");
  }
...
export async function getOrderRecords() {
  ...
  while (true) {
    for (const event of page.events ?? []) {
      ...
      const orderId = event.json?.order_id;
...
export async function getBuyerOrderRecords(buyerAddress) {
  if (!buyerAddress) return [];

  const orders = await getOrderRecords();
  const buyer = buyerAddress.toLowerCase();

  return orders.filter((order) => order.buyer?.toLowerCase() === buyer);
}

export async function getSellerOrderRecords(sellerAddress) {
  if (!sellerAddress) return [];

  const orders = await getOrderRecords();
  const seller = sellerAddress.toLowerCase();

  return orders.filter((order) => order.seller?.toLowerCase() === seller);
}

export async function getOrderRecordByEscrow(escrowId) {
  if (!escrowId) return null;

  const orders = await getOrderRecords();
  const target = escrowId.toLowerCase();
